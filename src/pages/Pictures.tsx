  ...
  const columns: Column[] = [
    {
      ...
      sortable: true,
      render: (value) => (
        <span className={`px-2 py-1 rounded border text-xs font-medium ${getErrorTypeColor(value)}`}>
      ...
      <Layout title="Pictures with Errors">
        <div className="w-full px-6 py-6">
          <Card className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-slate-900 mb-2">Error Loading Picture Errors</h3>
      ...
      <div className="w-full px-6 py-6 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600 mt-1">Manage and fix product pictures that encountered upload errors</p>
          </div>
          ...
          <div className="lg:col-span-3 space-y-6">
                    ...
                    {errorTypes.map(type => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className={`px-2 py-1 rounded border text-xs font-medium ${getErrorTypeColor(type)}`}>
                            {type.replace('_', ' ').toUpperCase()}
                          </span>
                          <span className="text-lg font-bold text-slate-900">{errorTypeCounts[type]}</span>
                        </div>
                        <div className="text-sm text-slate-600">
                          {((errorTypeCounts[type] / totalErrors) * 100).toFixed(1)}% of total errors
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {!isLoading && pictureErrors.length === 0 && (
              <Card className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-slate-900 mb-2">No Picture Errors Found</h3>
                <p className="text-slate-600 mb-6">Great! All product pictures have been uploaded successfully without any errors.</p>
            ...
            {isLoading && (
              <Card className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-slate-900 mb-2">Loading Picture Errors</h3>
                <p className="text-slate-600">Please wait while we fetch the picture error information...</p>
              </Card>
            )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <ul className="text-sm text-slate-600 space-y-1">
                      <li>• <strong>Upload Failed:</strong> Network or server issues during upload</li>
                      <li>• <strong>Format Error:</strong> Unsupported file format or corrupted file</li>
                      <li>• <strong>Size Error:</strong> File too large or dimensions incorrect</li>
                      <li>• <strong>Corruption:</strong> File data is corrupted or incomplete</li>
                    </ul>
                  </div>
                  <div>
