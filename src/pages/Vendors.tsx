...
export default function Vendors() {
  ...
  const performSearch = async (brandSearch: string) => {
    try {
      ...
      const uniqueSeasons = Array.from(new Set(data.purchaseOrders.map(order => order.purchaseOrderSeason).filter(Boolean))).sort();
  ...
  const getOrderDateStatus = (order: VendorOrder, isStartShip: boolean = false): 'normal' | 'warning' | 'danger' => {
    const today = new Date();
  ...
  const orderColumns: Column[] = [
    ...
    },
    {
      ...
      sortable: true,
      render: (value, order) => {
        const isOverpaid = value - order.purchaseOrderTotalCredits > order.purchaseOrderTotalItemsCost || value > order.totalOrderInvoices;
        return (
  ...
  const totalBalance = filteredOrders.reduce((sum, order) => sum + order.purchaseOrderBalanceDue, 0);
          ...
          <div className="lg:col-span-3 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <FormField label="Style Name">
                    <Input
                      ...
                      onChange={(e) => setSearchFilters(prev => ({ ...prev, styleName: e.target.value }))}
                      placeholder="Any"
                    />
                  </FormField>
                  <FormField label="Style Number">
                    <Input
                      ...
                      onChange={(e) => setSearchFilters(prev => ({ ...prev, styleNumber: e.target.value }))}
                      placeholder="Any"
                    />
                  </FormField>
            ...
            {selectedBrand !== 'Any' && (
              <>
                  ...
                  <Card>
                    <div className="space-y-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <h3 className="text-lg font-semibold text-slate-900">Brand: {selectedBrand}</h3>
                      </div>
                      <div className="flex items-center space-x-2">
                        <DollarSign className="w-5 h-5 text-slate-400" />
                        <span className="font-semibold text-slate-900">Open Balance:</span>
                        <span className="text-lg font-bold text-slate-900">{formatCurrency(totalBalance)}</span>
                      </div>
                    </div>
                  </Card>
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex-1">
                                  <div className="flex items-center space-x-2 text-sm">
                                    ...
                                    <span className="font-semibold">{formatCurrency(reminder.vendorAmountToPay)}</span>
                                  </div>
                                  <p className="text-sm text-slate-600 mt-1">{reminder.vendorAmoutToPayDescription}</p>
                                  <p className="text-xs text-slate-500">{reminder.vendorAmountToPayMethod}</p>
                                </div>
                                <Button 
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <FormField label="Date">
                              <Input
                                ...
                                onChange={(e) => updateNewReminder(index, 'vendorAmountToPayDate', e.target.value)}
                              />
                            </FormField>
                            <FormField label="Amount">
                              <Input
                                ...
                                onChange={(e) => updateNewReminder(index, 'vendorAmountToPay', parseFloat(e.target.value) || 0)}
                                placeholder="0.00"
                              />
                            </FormField>
                            <FormField label="Description">
                              <Input
                                ...
                                onChange={(e) => updateNewReminder(index, 'vendorAmoutToPayDescription', e.target.value)}
                            ...
                            <FormField label="Method">
                              <div className="flex space-x-2">
                                <select
                                  ...
                                  onChange={(e) => updateNewReminder(index, 'vendorAmountToPayMethod', e.target.value)}
                ...
                {seasons.length > 0 && (
                  <Card>
                    <div className="flex items-center space-x-4">
                      <label htmlFor="seasonSelect" className="text-sm font-semibold text-slate-700">
                        Filter by Season:
                      </label>
                      <select
                        id="seasonSelect"
                ...
                <DataTable
                  ...
                  emptyMessage={isSearching ? "Searching orders..." : "No orders found for this vendor"}
                />
              </>
            )}
            ...
            {selectedBrand === 'Any' && (
              <Card className="text-center py-12">
                ...
                <p className="text-slate-600 mb-6">Choose a vendor from the dropdown above to view their orders and manage payment reminders.</p>
              </Card>
            )}
          </div>
